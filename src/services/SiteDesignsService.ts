...
export interface ISiteDesignsService {
    ...
    saveSiteDesign(siteDesign: ISiteDesign): Promise<ISiteDesign>;
...
export class SiteDesignsService implements ISiteDesignsService {
    ...
    public getSiteScriptFromList(listUrl: string): Promise<ISiteScriptContent> {
        let listAbsoluteUrl = listUrl.indexOf('https://') == 0 || listUrl.indexOf('http://') == 0 ? listUrl : this._getEffectiveUrl(listUrl);
        return this._restRequest('/_api/Microsoft_SharePoint_Utilities_WebTemplateExtensions_SiteScriptUtility_GetSiteScriptFromList', {
            listUrl: listAbsoluteUrl
        }).then(r => JSON.parse(r.value) as ISiteScriptContent);
    }
}
